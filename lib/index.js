...
const validateReducerReturn = (reducer, defaultState) => (...args) => {
    const res = reducer(...args);
    if (res !== defaultState) {
        console.log('res', res, JSON.stringify(res, null, 2));
        console.log('defaultState', defaultState);
        throw new Error('Reducer must return the defaultState entity');
    }
    return res;
};
...
export const bbCreateStore = (storeCreator = createStore) => (bbReturningReducer, defaultState, enhancer = noop) => {
    ...
    if (!defaultState) {
        throw new Error('bbCreateStore()(reducer, defaultState) - defaultState is missing.');
    }

    const combinedBBEntities = bbCombineEntities(defaultState);
    ...
    const store = storeCreator(
        validateReducerReturn(bbReturningReducer, combinedBBEntities),
        combinedBBEntities,
        composedEnhancers
    );
