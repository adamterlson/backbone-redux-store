...
export const jsonReturningReducer = reducer => (...args) => {
    const res = reducer(...args);
    if (typeof res === 'undefined') {
        throw new Error('Reducer must have a return value');
    }
    if (res.toJSON) {
        return res.toJSON();
    }
    // I fear the performance here....
 ...
 */
export const bbCreateStore = (storeCreator = createStore) => (bbEntityOrReducer, defaultState, enhancer = noop) => {
    ...
    const store = storeCreator(jsonReturningReducer(bbReturningReducer), defaultState, composedEnhancers);

    if (combinedBBEntities) {
        combinedBBEntities.on(BB_COMM_EVENT, (action) => store.dispatch(action));
    }
